...
- the QP's `UNBOUNDED` verdict has to survive its own certificate: the ray is
  re-verified over the caller's `Q`/`A`/`b` by an independent checker, with
  non-positive curvature decided in double-double rather than by a tolerance
  band (`tools/qp_ray_test.c`, `docs/CURV_PS_PLAN.md` 1.12).

...
MiniZinc spec (docs.minizinc.dev, 4.1.11.2): the modulo result, if non-zero,
has the sign of its FIRST operand (the dividend), and
    x = (x div y) * y + (x mod y)
which pins x div y to truncation toward zero (the same as C's / and %).
This is NOT Python's floor semantics: -7 div 3 = -2, -7 mod 3 = -1,
7 mod -3 = 1.

Generates small random .fzn instances biased toward edges (negative
dividends, negative divisors, duplicate set members, wide/inexact powers)
and checks ./fznsolve's verdict:
...
def mzn_divmod(a, k):
    """MiniZinc semantics: quotient truncates toward zero; the remainder, if
    non-zero, has the sign of the dividend (same as C's / and %)."""
    q = abs(a) // abs(k)
    if (a < 0) != (k < 0):
        q = -q
    r = a - k * q
...
def gen_instance(rng, unbounded_r=False):
    """Return (text, checks, assigns, witness_iter, opt): checks are the
    ...
    semantics, witness_iter enumerates candidates, opt is
    None/'minimize'/'maximize' on x.  `unbounded_r` makes the result var a
    bare `var int` (honesty-downgrade paths); otherwise it keeps a bounded
    declared box like real mzn2fzn output (stronger signal)."""
    ...
    if kind in ("div", "mod"):
        ...
        # sometimes duplicate a member: sets must behave as sets
        if rng.random() < 0.3:
            raw = raw + raw[:1]
        rng.shuffle(raw)
        S = "{" + ", ".join(map(str, raw)) + "}"
    ...
    def witness_iter():
        ...
        for x in range(xlo, xhi + 1):
            for xs in xs_options:
                ...
                if have_r:
                    if kind == "div":
                        v["r"] = mzn_divmod(x, K)[0]
                    elif kind == "mod":
                        v["r"] = mzn_divmod(x, K)[1]
                    else:
                        v["r"] = x ** exp
...
EDGE_CASES = [
    ...
    # MiniZinc TRUNCATION semantics: -7 div 3 = -2, -7 mod 3 = -1 (remainder
    # has the sign of the dividend), 7 mod -3 = 1.
    ...
    ("var int: r :: output_var;\nconstraint int_mod(-7, 3, r);\nsolve satisfy;\n",
     "r = -1;"),
    # int_pow must not pin an inexact/overflowed double: 3^40 is a valid
    # integer but > 2^53 -> honest UNKNOWN, never UNSAT; 2^10 is exact.
    ...
    ("var 1..300: y :: output_var;\nconstraint int_eq(y, 265);\n"
     ...
     "y = 265;"),
    # a singleton set outside the declared domain is UNSAT, not a new domain
    ("var 0..5: x :: output_var;\nconstraint set_in(x, {8});\nsolve minimize x;\n",
     "UNSATISFIABLE"),
    ...
    ("var int: z :: output_var;\nconstraint int_lin_eq([1], [z], 20000000000);\nsolve satisfy;\n",
     "UNKNOWN"),
    # count over a variable target: exact (previously silently counted zeros)

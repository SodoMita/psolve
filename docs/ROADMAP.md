...
- **Unbounded QPs ground to the iteration limit.**  Added a recession-direction
  ...
  4,000 wasted iterations.  Both halves are judged one-sided -- the curvature's
  *sign* over a double-double evaluation, the rows over each row's own
  cancellation scale (CURV_PS_PLAN 1.12) -- so a failed certificate degrades to
  the old iteration limit rather than risking a wrong answer.

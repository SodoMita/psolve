...
Ported to `arena/cp-engine-correctness` (2026-08-13) with one branch
adaptation: this branch's `mip.c` predates the phase4 arena API, so the
early-return frees plain `free()` and — caught on review — sets
`res->nodes = 0`, which the FlatZinc bridge reads unconditionally (the
upstream block left it unset on the early-return path).  Independently

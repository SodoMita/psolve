...
# The QP's INFEASIBLE claim is printed by the CLI only when psv_cert_check
# confirms the Farkas vector over the model's own A and b.  The checker is tested
# adversarially rather than by acceptance, because its whole job is refusing the
# claims that look right: sign-flipped, zeroed, non-finite, checked against
# another rhs, row-cleared, missing payload -- and a hand-rolled "proof" against
# a system that is feasible, which is the class of bug a producer-side verifier
# would otherwise share with itself.
gcc -O2 -march=native -I src tools/qp_cert_test.c src/qp.c src/cert.c src/solver.c \
    src/splu.c src/lu.c src/kernels.c src/err.c -o /tmp/qp_cert_test -lm
/tmp/qp_cert_test 20
# End to end: x <= 1 together with -x <= -2 is empty and must come back proven;
# the same model with -x <= 2 is feasible and must not claim a proof.
printf '1 2\n0\n2\n1\n-1\n1\n-2\n' > /tmp/qp_empty.qp
printf '1 2\n0\n2\n1\n-1\n1\n2\n'  > /tmp/qp_feas.qp
out=$(/tmp/qpsolve /tmp/qp_empty.qp)
case "$out" in
  *"PROVEN 1"*) : ;;
  *) echo "  qpsolve did not confirm an empty-system proof"; echo "$out"; exit 1 ;;
esac
case "$out" in
  *"PROVEN 0"*) echo "  qpsolve printed an unconfirmed proof"; exit 1 ;;
  *) : ;;
esac
out=$(/tmp/qpsolve /tmp/qp_feas.qp)
case "$out" in
  *"PROVEN"*) echo "  a feasible model claimed an infeasibility proof"; exit 1 ;;
  *) : ;;
esac
